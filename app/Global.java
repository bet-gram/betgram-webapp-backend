...
public class Global extends GlobalSettings {
    ...
    public void onStart(Application app) {
        ...
        List<Ground> grounds = new Model.Finder(Long.class,  Ground.class).all();
        List<Team> teams = new Model.Finder(String.class, Team.class).all();
        List<Match> matches = new Model.Finder(Long.class, Match.class).all();
        List<Climate> climates = new Model.Finder(String.class, Climate.class).all();
        List<Metrics> metrics = new Model.Finder(String.class, Metrics.class).all();
        List<HistoricData> historics = new Model.Finder(String.class, HistoricData.class).all();

        if(grounds.size() + teams.size() + matches.size() + climates.size() + metrics.size() + historics.size() == 0) {
